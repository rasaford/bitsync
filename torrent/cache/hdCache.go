...
// HdCacheProvider creates an HD cache for each torrent.
...
func NewHdCacheProvider(capacity int) Provider {
	os.Mkdir(filepath.FromSlash(os.TempDir()+"/taipeitorrent"), 0777)
	rc := &HdCacheProvider{capacity, make(map[string]*HdCache)}
	return rc
}

func (r *HdCacheProvider) NewCache(infohash string, numPieces int, pieceSize int64, torrentLength int64, underlying file.FileStore) file.FileStore {
	i := uint32(1)
	rc := &HdCache{pieceSize: pieceSize, atimes: make([]time.Time, numPieces), boxExists: *bitset.NewBitset(numPieces),
